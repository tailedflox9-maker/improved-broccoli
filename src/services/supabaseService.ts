...
export const getProfile = async (): Promise<Profile> => {
  ...
  if (error) { console.error("Error calling get_my_profile RPC:", error); throw new Error("Could not fetch user profile."); }
  if (!data || data.length === 0) { throw new Error("User profile not found in the database."); }
...
// --- MODIFIED: This now performs a SOFT delete for users ---
export const softDeleteConversation = async (id: string) => {
  const { error } = await supabase.from('conversations').update({ is_deleted_by_user: true }).eq('id', id);
  if (error) throw error;
};
...
export const getAllUsers = async (): Promise<Profile[]> => {
    try {
        ...
        if (directError) { throw new Error(`Unable to fetch users. Error: ${directError.message}`); }
        return directData as Profile[];
    } catch (error: any) { throw new Error(`Unable to fetch user data: ${error.message}`); }
};

// --- NEW: Admin function to get ALL conversations for a user, including soft-deleted ones ---
export const getAllConversationsForUser_Admin = async (userId: string): Promise<Conversation[]> => {
  ...
  if (error) throw new Error(`Failed to fetch user conversations for admin: ${error.message}`);
  return data as Conversation[];
};

// --- NEW: Admin function to HARD delete a conversation permanently ---
export const hardDeleteConversation_Admin = async (id: string): Promise<void> => {
  const adminClient = getAdminClient();
  const { error } = await adminClient.from('conversations').delete().eq('id', id);
  if (error) throw new Error(`Failed to hard delete conversation: ${error.message}`);
};
...
export const createUser = async (userData: { email: string; password: string; full_name: string; role: 'student' | 'teacher' }): Promise<any> => {
  try {
    ...
    const { data: authData, error: authError } = await adminClient.auth.admin.createUser({
      email: userData.email.trim(), password: userData.password.trim(), email_confirm: true,
...
export const assignTeacherToStudent = async (teacherId: string, studentId: string): Promise<void> => {
  ...
  } catch (error: any) { throw new Error(error.message || 'An unexpected error occurred while assigning the teacher.'); }
};
...
export const getStudentsForTeacher = async (teacherId: string): Promise<Profile[]> => {
  ...
  if (error) { console.error('Error fetching students for teacher:', error); throw error; }
...
export const getFlaggedMessagesForTeacher = async (teacherId: string): Promise<FlaggedMessage[]> => {
    ...
    if (error) { console.error('Error fetching flagged messages:', error); throw error; }
